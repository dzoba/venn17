...
theorem diagram_no_triple_intersections (i j k : Fin 17) (hij : i ≠ j) (hik : i ≠ k) (hjk : j ≠ k)
    (x : Diagram) (hi : x ∈ diagramCurve i) (hj : x ∈ diagramCurve j) : x ∉ diagramCurve k :=
  encoded_no_triple suppliedModel.oriented 131072 supplied_corners_bounded
...
theorem crossingPoint_encoded (f : Fin suppliedModel.oriented.size) :
    (diagramEncodedHomeomorph (crossingPoint f)).val = Coordinates.vertex (131072+f.val) := by
...
theorem diagram_crossing_iff (x : Diagram) :
    (∃ i j : Fin 17, i ≠ j ∧ x ∈ diagramCurve i ∧ x ∈ diagramCurve j) ↔
      ∃ f : Fin suppliedModel.oriented.size, x = crossingPoint f := by
  constructor
  · rintro ⟨i,j,hij,hi,hj⟩
    obtain ⟨f,hf⟩ := encodedCurve_intersection_center suppliedModel.oriented 131072
    ...
    exact hf
  · rintro ⟨f,rfl⟩
    obtain ⟨i,j,hij,hi,hj⟩ := center_has_two_curves suppliedModel.oriented 131072 17
      supplied_curve_labels_verified supplied_crossing_labels_verified f
    refine ⟨i,j,hij,?_,?_⟩
    · change (diagramEncodedHomeomorph (crossingPoint f)).val ∈ encodedCurve suppliedModel.oriented 131072 i.val
      rw [crossingPoint_encoded]
      exact hi
    · change (diagramEncodedHomeomorph (crossingPoint f)).val ∈ encodedCurve suppliedModel.oriented 131072 j.val
      rw [crossingPoint_encoded]
      exact hj

theorem diagram_crossing_exactly_two (f : Fin suppliedModel.oriented.size) :
    ∃ i j : Fin 17, i ≠ j ∧ ∀ k : Fin 17,
