...
theorem sector_triangle_subset (k : Fin 17) (f : Fin suppliedModel.oriented.size)
    (hf : sectorColor f = k) (j : Fin 4) : diagramQuad.triangle f j ⊆ sectorSpace k := by
  intro x hx
  exact mem_iUnion.mpr ⟨f, mem_iUnion₂.mpr ⟨hf, j, hx⟩⟩

theorem sector_centers_joined (k : Fin 17) (f g : Fin suppliedModel.oriented.size)
...
theorem sector_walk_joined (k : Fin 17) {u v : Nat}
    (hw : GraphProof.Walk suppliedModel.dual (fun f => suppliedSectorColors.getD f 17 == k.val) u v)
  ...
  | @step u v w ha hedge hw ih =>
      ...
      have hav : (suppliedSectorColors.getD v 17 == k.val) = true := by
        cases hw with
        | refl _ h => exact h
        | step h _ _ => exact h
...
theorem sectorSpace_pathConnected (k : Fin 17) : IsPathConnected (sectorSpace k) := by
...
theorem diagramSector_pathConnected (k : Fin 17) : IsPathConnected (diagramSector k) :=
