...
angles +2π/17 and -2π/17. This identifies the local generator; extending it
...
def encodedRotation : Equiv.Perm Nat :=
  (Coordinates.labelEquiv 131072 suppliedModel.oriented.size).symm.trans
    (diagramAutomorphism.labels.trans (Coordinates.labelEquiv 131072 suppliedModel.oriented.size))

theorem encodedRotation_step (x : Nat) (hx : x < 131072 + suppliedModel.oriented.size) :
    encodedRotation x = encodedRotationStep suppliedModel.oriented x := by
  by_cases h : x < 131072
  · have he : (Coordinates.labelEquiv 131072 suppliedModel.oriented.size).symm x = .inl x := by
      apply (Coordinates.labelEquiv 131072 suppliedModel.oriented.size).injective
      simp only [Equiv.apply_symm_apply, Coordinates.labelEquiv_region _ _ _ h]
    change Coordinates.labelEquiv 131072 suppliedModel.oriented.size
      (diagramAutomorphism.labels ((Coordinates.labelEquiv 131072 suppliedModel.oriented.size).symm x)) = _
    rw [he]
    change Coordinates.labelEquiv 131072 suppliedModel.oriented.size (.inl (regionStep x)) = _
    rw [Coordinates.labelEquiv_region _ _ _ (regionStep_lt h)]
    simp only [encodedRotationStep, encodedRotationStepWith, if_pos h]
  · let f : Fin suppliedModel.oriented.size := ⟨x - 131072, by omega⟩
    have he : (Coordinates.labelEquiv 131072 suppliedModel.oriented.size).symm x = .inr f := by
      apply (Coordinates.labelEquiv 131072 suppliedModel.oriented.size).injective
    ...
    change Coordinates.labelEquiv 131072 suppliedModel.oriented.size
      (diagramAutomorphism.labels ((Coordinates.labelEquiv 131072 suppliedModel.oriented.size).symm x)) = _
    rw [he]
    change 131072 + (rotationWitnesses suppliedModel.oriented)[f.val]! = _
...
theorem poleLinkActionCheck_sound (fs : Array Face) (stars : LinkProof.Stars) (pole step : ℕ)
    ...
    (stars.getD pole #[]).size = 34 ∧ ∀ j : Fin (stars.getD pole #[]).size,
      rowVertices fs 131072 (stars.getD pole #[]) j < 131072 + fs.size ∧
      encodedRotationStep fs (rowVertices fs 131072 (stars.getD pole #[]) j) =
        rowVertices fs 131072 (stars.getD pole #[]) (Coordinates.cyclicNext^[step] j) := by
...
def poleIndex (p : Fin 2) : Fin suppliedLinkStars.size :=
  ⟨if p = 0 then 0 else 131071, by rw [supplied_link_rows.1]; split_ifs <;> omega⟩

def poleLinkStep (p : Fin 2) : ℕ := if p = 0 then 2 else 32

theorem pole_link_action (p : Fin 2) :
    let row := suppliedLinkStars.getD (poleIndex p).val #[]
    row.size = 34 ∧ ∀ j : Fin row.size,
      encodedRotation (rowVertices suppliedModel.oriented 131072 row j) =
        rowVertices suppliedModel.oriented 131072 row (Coordinates.cyclicNext^[poleLinkStep p] j) := by
...
theorem pole_link_circle_rotation (p : Fin 2) (z : Circle) :
    Coordinates.reindex encodedRotation (supplied_dart_link_circle (poleIndex p) z).val =
      (supplied_dart_link_circle (poleIndex p)
        (Circle.exp (2 * Real.pi / 34 * poleLinkStep p) * z)).val := by
  ...
  have hg := Coordinates.labeledPolygonHomeomorph_rotation encodedRotation
    (rowVertices suppliedModel.oriented 131072 (suppliedLinkStars.getD (poleIndex p).val #[]))
    (LinkProof.rowCheck_sound _ _ _ _ hr).2.1 (LinkProof.rowCheck_sound _ _ _ _ hr).1
    (poleLinkStep p) h.2 z
  have he : (↑(suppliedLinkStars.getD (poleIndex p).val #[]).size : ℝ) = 34 := by
    exact_mod_cast h.1
  rw [he] at hg
  exact hg
...
theorem zero_pole_link_rotation (z : Circle) :
    Coordinates.reindex encodedRotation (supplied_dart_link_circle (poleIndex 0) z).val =
      (supplied_dart_link_circle (poleIndex 0) (Circle.exp (2 * Real.pi / 17) * z)).val := by
  have h := pole_link_circle_rotation 0 z
  change _ = (supplied_dart_link_circle (poleIndex 0) (Circle.exp (2 * Real.pi / 34 * 2) * z)).val at h
  rw [show 2 * Real.pi / 34 * 2 = 2 * Real.pi / 17 by ring] at h
  exact h

theorem one_pole_link_rotation (z : Circle) :
    Coordinates.reindex encodedRotation (supplied_dart_link_circle (poleIndex 1) z).val =
      (supplied_dart_link_circle (poleIndex 1) (Circle.exp (-(2 * Real.pi / 17)) * z)).val := by
  have h := pole_link_circle_rotation 1 z
  change _ = (supplied_dart_link_circle (poleIndex 1) (Circle.exp (2 * Real.pi / 34 * 32) * z)).val at h
  rw [show 2 * Real.pi / 34 * 32 = -(2 * Real.pi / 17) + 2 * Real.pi by ring,
