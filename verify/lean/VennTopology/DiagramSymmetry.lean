...
theorem supplied_graph_size : suppliedModel.graph.size = 131072 := by native_decide
...
theorem diagram_rotation_period (x : Diagram) : diagramRotation^[17] x = x :=
  diagramAutomorphism.homeomorph_period 17 region_period
    (face_period _ supplied_rotation_verified) x

noncomputable def patternPoint (v : Fin 131072) : Diagram := by
...
theorem diagram_rotation_pattern (n : Nat) (v : Fin 131072) :
...
theorem region_no_short_period :
    ∀ k : Fin 17, k.val ≠ 0 → steps regionStep k.val 1 ≠ 1 := by decide

/-- The homeomorphism has exact order 17: every smaller positive power moves
the point carrying membership pattern 1. -/
theorem diagram_rotation_no_short_period (k : Nat) (hk : 0 < k) (hk17 : k < 17) :
...
theorem diagram_rotation_fixes_one :
    diagramRotation (patternPoint 131071) = patternPoint 131071 := by
  apply Subtype.ext
  exact diagram_rotation_pattern 1 131071

theorem diagram_poles_distinct : patternPoint 0 ≠ patternPoint 131071 := by
  exact fun h => (by decide : (0 : Fin 131072) ≠ 131071) (patternPoint_injective h)
