...
theorem diagram_cells_bounded (t : Fin (4 * suppliedModel.oriented.size)) (i : Nat)
    (hi : i ∈ encodedCells suppliedModel.oriented 131072 t) :
    i < 131072 + suppliedModel.oriented.size :=
...
theorem diagram_encoded_cell_card (t : Fin (4 * suppliedModel.oriented.size)) :
    (encodedCells suppliedModel.oriented 131072 t).card = 3 :=
...
def diagramFiniteTriangulation : GeometricTriangulation Diagram :=
  let tr := finiteTriangulation (encodedCells suppliedModel.oriented 131072)
    (131072 + suppliedModel.oriented.size) diagram_cells_bounded diagram_encoded_cell_card
  { tr with homeo := tr.homeo.trans diagramEncodedHomeomorph.symm }

theorem diagram_finite_vertex_count :
    Fintype.card diagramFiniteTriangulation.Vertex = 262142 := by
  change Fintype.card (Fin (131072 + suppliedModel.oriented.size)) = _
  rw [Fintype.card_fin, supplied_oriented_face_count]

theorem diagram_finite_edge_count : diagramFiniteTriangulation.edges.card = 786420 := by
  have h := Nat.card_congr (finiteTriangulationEdgeEquiv
    (encodedCells suppliedModel.oriented 131072) (131072 + suppliedModel.oriented.size)
    diagram_cells_bounded diagram_encoded_cell_card)
  have he := encoded_edge_count suppliedModel.oriented 131072 suppliedLinkStars
...
theorem diagram_finite_triangle_count : diagramFiniteTriangulation.faces.card = 524280 := by
  have h := finiteTriangulation_faces_card (encodedCells suppliedModel.oriented 131072)
    (131072 + suppliedModel.oriented.size) diagram_cells_bounded diagram_encoded_cell_card
