...
def ambientSide (i : Fin 17) (b : Bool) : Set (Quad.Ambient Nat (Fin suppliedModel.oriented.size)) :=
  diagramQuad.space ∩ {x | 0 < sideSign b * diagramQuad.bitField 131072 i.val x}

def diagramSide (i : Fin 17) (b : Bool) : Set Diagram :=
  {x | 0 < sideSign b * diagramBitField i x}

theorem signed_halfspace_convex (i : Fin 17) (b : Bool) :
    Convex ℝ {x | 0 < sideSign b * diagramQuad.bitField 131072 i.val x} := by
  exact (convex_Ioi (0 : ℝ)).linear_preimage (sideSign b • diagramQuad.bitField 131072 i.val)

theorem side_triangle_joined (i : Fin 17) (b : Bool)
    ...
    (hsx : 0 < sideSign b * diagramQuad.bitField 131072 i.val x)
    (hsy : 0 < sideSign b * diagramQuad.bitField 131072 i.val y) :
...
theorem side_point_joined_vertex (i : Fin 17) (b : Bool)
  ...
  have hpos : 0 < sideSign b * bitSign i.val (diagramQuad.corner f k) ∨
    ...
    · left
      ...
      have he : diagramQuad.bitField 131072 i.val x = diagramQuad.faceSign i.val f := by
    ...
    · rw [hzero,mul_zero,zero_add] at hf
      ...
      have he : sideSign b * diagramQuad.bitField 131072 i.val x =
      ...
      have hp : 0 < sideSign b * diagramQuad.bitField 131072 i.val x := hx.2
      rw [he] at hp
  ...
  · refine ⟨⟨_,hb k⟩,(sideActive_iff_sign _ _ _).mpr hp,?_⟩
    ...
    rwa [diagramQuad.bitField_region 131072 i.val _ (hb k)]
  · refine ⟨⟨_,hb (Quad.next k)⟩,(sideActive_iff_sign _ _ _).mpr hp,?_⟩
    ...
    rwa [diagramQuad.bitField_region 131072 i.val _ (hb (Quad.next k))]

theorem side_vertex_mem (i : Fin 17) (b : Bool) (v : Nat) (hv : v < 131072)
    (ha : sideActive i.val b v = true) : Quad.point (.inl v) ∈ ambientSide i b := by
  refine ⟨(patternPoint ⟨v,hv⟩).property,?_⟩
  change 0 < sideSign b * diagramQuad.bitField 131072 i.val (Quad.point (.inl v))
  rw [diagramQuad.bitField_region 131072 i.val v hv]
  exact (sideActive_iff_sign _ _ _).mp ha

theorem side_walk_joined (i : Fin 17) (b : Bool) {u v : Nat}
    (hw : GraphProof.Walk suppliedModel.graph (sideActive i.val b) u v) (hu : u < 131072) :
  ...
  | @step u v w ha he rest ih =>
    ...
    have hv : v < 131072 := hfv ▸ hb (Quad.next k)
...
theorem ambientSide_pathConnected (i : Fin 17) (b : Bool) : IsPathConnected (ambientSide i b) := by
  obtain ⟨r,hr,ha,hw⟩ := each_side_connected i.val i.isLt b
  have hr' : r < 131072 := by simpa only [supplied_graph_size] using hr
...
theorem diagramSide_pathConnected (i : Fin 17) (b : Bool) : IsPathConnected (diagramSide i b) := by
