...
def sectorCore (k : Fin 17) : Set Diagram := (⋃ l, ⋃ (_ : l ≠ k), diagramSector l)ᶜ

theorem sectorCore_open (k : Fin 17) : IsOpen (sectorCore k) :=
  (isClosed_iUnion_of_finite fun l => isClosed_iUnion_of_finite fun _ => diagramSector_closed l).isOpen_compl

theorem sectorCore_subset (k : Fin 17) : sectorCore k ⊆ diagramSector k := by
  intro x hx
...
theorem sector_diff_boundary_subset_core (k : Fin 17) :
...
theorem sector_frontier_subset (k : Fin 17) :
...
theorem meridianBoundaryLabel_lt (k l : Fin 17) (i : Fin 34) : meridianBoundaryLabel k l i < 131072 := by
  unfold meridianBoundaryLabel
  split <;> exact (meridian_labels_verified.2.1 _ _).1

theorem meridian_boundary_center_zero (k l : Fin 17) {x : Diagram}
    (hx : x ∈ diagramMeridianBoundary k l) (a : Nat) (ha : 131072 ≤ a) :
...
theorem crossingPoint_not_meridian_boundary (f : Fin suppliedModel.oriented.size) (k l : Fin 17) :
    crossingPoint f ∉ diagramMeridianBoundary k l := by
  intro hx
  have h := meridian_boundary_center_zero k l hx (131072+f.val) (by omega)
  rw [crossingPoint_encoded, vertex_self] at h
  norm_num at h

theorem crossingPoint_mem_sector_iff (f : Fin suppliedModel.oriented.size) (k : Fin 17) :
  ...
  · intro hx
    have he := encodedSector_positive_allowed k ((mem_diagramSector_iff_encoded k _).mp hx)
      (a := 131072+f.val) (by rw [crossingPoint_encoded, vertex_self]; norm_num)
...
theorem sector_face_exists (k : Fin 17) : ∃ f, sectorColor f = k := by
...
theorem sector_interior_nonempty (k : Fin 17) : (interior (diagramSector k)).Nonempty := by
...
def sectorOutsidePoint (k : Fin 17) : Diagram :=
  crossingPoint (Classical.choose (sector_face_exists (cyclicNext k)))

theorem sectorOutsidePoint_not_mem (k : Fin 17) : sectorOutsidePoint k ∉ diagramSector k := by
