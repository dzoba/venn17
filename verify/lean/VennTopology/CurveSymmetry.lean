...
/-- Character `i` moves to `i-1` modulo 17, as specified in the input. -/
def curveRotation : Equiv.Perm (Fin 17) := Equiv.addRight 16

@[simp] theorem curveRotation_val (i : Fin 17) :
    (curveRotation i).val = (i.val+16)%17 := rfl

theorem curve_rotation_period : ∀ i : Fin 17, curveRotation^[17] i = i := by decide

theorem bit_rotation_verified : ∀ (v : Fin 131072) (i : Fin 17),
    regionStep (v.val ^^^ 2^i.val) = regionStep v.val ^^^ 2^(curveRotation i).val := by
  native_decide

theorem rotated_edge_label {u v : Nat} (hu : u < 131072) (i : Fin 17) :
...
theorem diagram_rotated_edge_label (f : Fin suppliedModel.oriented.size) (k : Fin 4) (i : Fin 17) :
...
theorem diagram_rotation_mem_curve (x : Diagram) (i : Fin 17) :
...
theorem diagram_rotation_curve_image (i : Fin 17) :
...
noncomputable def diagramCurveRotation (i : Fin 17) :
