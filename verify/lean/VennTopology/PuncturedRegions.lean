...
theorem encoded_cells_erase_nonempty (t : Fin (4*suppliedModel.oriented.size)) (a : Nat)
    (_ha : a ∈ encodedCells suppliedModel.oriented 131072 t) :
    ((encodedCells suppliedModel.oriented 131072 t).erase a).Nonempty := by
...
def encodedRegionLinkCircle (v : Fin 131072) :
    Circle ≃ₜ Coordinates.link (encodedCells suppliedModel.oriented 131072) v.val :=
...
theorem encoded_region_link_pathConnected (v : Fin 131072) :
    IsPathConnected (Coordinates.link (encodedCells suppliedModel.oriented 131072) v.val) := by
  ...
  have he : (fun z => (encodedRegionLinkCircle v z).val) '' Set.univ =
      Coordinates.link (encodedCells suppliedModel.oriented 131072) v.val := by
    ext x
    constructor
...
theorem patternPoint_encoded (v : Fin 131072) :
...
theorem diagramRegion_punctured_pathConnected (v : Fin 131072) :
    IsPathConnected (diagramRegion v \ {patternPoint v}) := by
  have h := Coordinates.punctured_region_pathConnected
    (encodedCells suppliedModel.oriented 131072) 131072 v.val
