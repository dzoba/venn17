...
/-- The required positive generator, with angle exactly `2π/17`. -/
def rigidGenerator : Circle := Circle.exp (2 * Real.pi / 17)

theorem rigidGenerator_pow_seventeen : rigidGenerator ^ 17 = 1 := by
  rw [rigidGenerator, ← Circle.exp_nsmul]
  rw [nsmul_eq_mul]
  norm_num only [Nat.cast_ofNat]
  rw [show (17 : ℝ) * (2 * Real.pi / 17) = 2 * Real.pi by ring]
...
theorem rigidSphereRotation_period (x : SphereRepresentative) :
    (rigidSphereRotation rigidGenerator)^[17] x = x := by
  rw [rigidSphereRotation_iterate, rigidGenerator_pow_seventeen, rigidSphereRotation_one]
