...
theorem supplied_oriented_face_count : suppliedModel.oriented.size = 131070 := by
  native_decide
...
theorem diagram_geometric_vertex_count : Nat.card (GeometricFace diagramQuad 1) = 262142 := by
  have h := geometric_vertex_count suppliedModel.oriented 131072 supplied_corners_bounded supplied_region_coverage
  exact h.trans (by rw [supplied_oriented_face_count])

theorem diagram_geometric_edge_count : Nat.card (GeometricFace diagramQuad 2) = 786420 := by
  have h := geometric_edge_count suppliedModel.oriented 131072 suppliedLinkStars
...
theorem diagram_geometric_triangle_count : Nat.card (GeometricFace diagramQuad 3) = 524280 := by
  have h := geometric_triangle_count suppliedModel.oriented 131072
    supplied_corners_bounded supplied_distinct_corners_verified
  exact h.trans (by rw [supplied_oriented_face_count])
...
theorem diagram_triangulation_counts_verified :
    Nat.card (GeometricFace diagramQuad 1) = 262142 ∧
    Nat.card (GeometricFace diagramQuad 2) = 786420 ∧
    Nat.card (GeometricFace diagramQuad 3) = 524280 ∧
