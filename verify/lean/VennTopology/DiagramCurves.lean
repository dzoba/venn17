...
def diagramEncodedHomeomorph : Diagram ≃ₜ Coordinates.realization (encodedCells suppliedModel.oriented 131072) :=
...
def diagramCurve (i : Fin 17) : Set Diagram :=
  {x | (diagramEncodedHomeomorph x).val ∈ encodedCurve suppliedModel.oriented 131072 i.val}
...
/-- An actual circle homeomorphism for each of the 17 labelled curves. The range
covers all of that label's center-to-midpoint arms, with none omitted. -/
def diagramCurveHomeomorph (i : Fin 17) : Circle ≃ₜ diagramCurve i :=
  (encodedCurveHomeomorph suppliedModel.oriented 131072 i.val
    ...
    (diagramEncodedHomeomorph.subtype (p := fun x => x ∈ diagramCurve i)
      (q := fun x => x.val ∈ encodedCurve suppliedModel.oriented 131072 i.val)
      (fun _ => Iff.rfl)).symm

def diagramCurveMap (i : Fin 17) (z : Circle) : Diagram := (diagramCurveHomeomorph i z).val

theorem diagramCurveMap_continuous (i : Fin 17) : Continuous (diagramCurveMap i) :=
  continuous_subtype_val.comp (diagramCurveHomeomorph i).continuous

theorem diagramCurveMap_injective (i : Fin 17) : Function.Injective (diagramCurveMap i) :=
  Subtype.val_injective.comp (diagramCurveHomeomorph i).injective

theorem diagramCurveMap_range (i : Fin 17) : Set.range (diagramCurveMap i) = diagramCurve i := by
  ext x
  constructor
...
theorem diagramCurve_compact (i : Fin 17) : IsCompact (diagramCurve i) := by
  rw [← diagramCurveMap_range]
  exact isCompact_range (diagramCurveMap_continuous i)

theorem diagramCurve_closed (i : Fin 17) : IsClosed (diagramCurve i) :=
  (diagramCurve_compact i).isClosed

/-- All 17 concrete labelled curve sets are simple closed curves on `Diagram`.
Moving them to the plane still requires the global sphere/plane theorem. -/
theorem diagram_curves_are_embedded_circles :
    ∀ i : Fin 17, ∃ f : Circle → Diagram,
