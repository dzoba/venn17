...
theorem diagram_faceSign_cases (i : Fin 17) (f : Fin suppliedModel.oriented.size) :
  ...
  obtain ⟨a,b,hab,ha,hb⟩ := crossing_labels_alternate suppliedModel.oriented 131072 17
...
theorem diagram_edge_bitSign (f : Fin suppliedModel.oriented.size) (k : Fin 4) (i : Fin 17) :
  ...
  have hm : diagramQuad.corner f k ^^^ diagramQuad.corner f (Quad.next k) = 2^j.val := by
    apply (edgeMask_face _ 131072 f k).symm.trans
