...
theorem diagram_sector_iterate (n : Nat) (k : Fin 17) :
...
theorem sector_index_iterate (k : Fin 17) : cyclicNext^[k.val] (0 : Fin 17) = k := by
  decide +revert

def sectorFromZero (k : Fin 17) : diagramSector 0 ≃ₜ diagramSector k :=
...
def equivariantSectorDisk (k : Fin 17) : diagramSector k ≃ₜ PlaneClosedDisk :=
  (sectorFromZero k).symm.trans (sectorDiskHomeomorph 0)

theorem equivariantSectorDisk_boundary (k : Fin 17) (z : UnitCircle) :
...
theorem sectorFromZero_next (k : Fin 17) (x : diagramSector 0) :
    (sectorFromZero (cyclicNext k) x).val = diagramRotation (sectorFromZero k x).val := by
  rw [sectorFromZero_apply, sectorFromZero_apply]
  change diagramRotation^[(k.val+1)%17] x.val = diagramRotation (diagramRotation^[k.val] x.val)
  rw [(show Function.IsPeriodicPt diagramRotation 17 x.val from diagram_rotation_period x.val).iterate_mod_apply]
  exact Function.iterate_succ_apply' _ _ _

def sectorRotatePoint (k : Fin 17) (x : diagramSector k) : diagramSector (cyclicNext k) :=
...
to the next sector. Period 17 proves the last-to-first case as well. -/
theorem equivariantSectorDisk_rotation (k : Fin 17) (x : diagramSector k) :
