...
theorem supplied_corners_bounded : cornersCheck suppliedModel.oriented 131072 = true := by
...
def diagram_geometric_link_circle (a : Nat ⊕ Fin suppliedModel.oriented.size)
    (ha : Coordinates.labelEquiv 131072 suppliedModel.oriented.size a < suppliedLinkStars.size) :
    Circle ≃ₜ diagramQuad.geometricLink a :=
  (supplied_dart_link_circle ⟨_, ha⟩).trans
    (geometricLinkDartHomeomorph suppliedModel.oriented 131072
