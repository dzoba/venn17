...
def puncturedCurve (i : Fin 17) : Set PuncturedDiagram := Subtype.val ⁻¹' diagramCurve i
def puncturedRegion (v : Fin 131072) : Set PuncturedDiagram := Subtype.val ⁻¹' diagramRegion v

theorem pole_avoids_curves (i : Fin 17) : patternPoint 0 ∉ diagramCurve i := by
  have h := region_subset_complement 0 (patternPoint_mem_region 0)
  exact fun hi => h (mem_iUnion.mpr ⟨i,hi⟩)

def puncturedCurveHomeomorph (i : Fin 17) : diagramCurve i ≃ₜ puncturedCurve i where
...
def puncturedCurveCircle (i : Fin 17) : Circle ≃ₜ puncturedCurve i :=
  (diagramCurveHomeomorph i).trans (puncturedCurveHomeomorph i)

theorem punctured_curves_are_embedded_circles (i : Fin 17) :
...
theorem puncturedRegion_pathConnected (v : Fin 131072) : IsPathConnected (puncturedRegion v) := by
...
def puncturedComplement : Set PuncturedDiagram := (⋃ i : Fin 17, puncturedCurve i)ᶜ
def puncturedComplementRegion (v : Fin 131072) : Set puncturedComplement :=
...
theorem punctured_complement_unique_region (x : PuncturedDiagram) :
    x ∈ puncturedComplement ↔ ∃! v : Fin 131072, x ∈ puncturedRegion v := by
...
theorem puncturedComplementRegion_isClopen (v : Fin 131072) :
...
theorem puncturedComplementRegion_pathConnected (v : Fin 131072) :
...
def puncturedComponentsEquiv : ConnectedComponents puncturedComplement ≃ Fin 131072 :=
...
theorem punctured_complement_component_count : Nat.card (ConnectedComponents puncturedComplement) = 131072 := by
...
theorem punctured_rotation_period (x : PuncturedDiagram) : puncturedRotation^[17] x = x := by
  apply Subtype.ext
  rw [punctured_rotation_iterate,diagram_rotation_period]

theorem punctured_rotation_curve_image (i : Fin 17) :
...
theorem punctured_rotation_region_image (v : Fin 131072) :
...
theorem punctured_rotation_fixed_iff (x : PuncturedDiagram) :
    puncturedRotation x = x ↔ x.val = patternPoint 131071 := by
...
theorem punctured_rotation_no_short_period (k : Nat) (hk : 0 < k) (hk17 : k < 17) :
    ∃ x : PuncturedDiagram, puncturedRotation^[k] x ≠ x := by
  let x : PuncturedDiagram := ⟨patternPoint 1,fun he =>
    (by decide : (1 : Fin 131072) ≠ 0) (patternPoint_injective he)⟩
  refine ⟨x,fun he => ?_⟩
...
theorem punctured_crossing_iff (x : PuncturedDiagram) :
  ...
  change (∃ i j : Fin 17, i ≠ j ∧ x.val ∈ diagramCurve i ∧ x.val ∈ diagramCurve j) ↔ _
  rw [diagram_crossing_iff]
  apply exists_congr
  intro f
  exact ⟨fun h => Subtype.ext h,fun h => congrArg Subtype.val h⟩

theorem punctured_no_triple_intersections (i j k : Fin 17)
