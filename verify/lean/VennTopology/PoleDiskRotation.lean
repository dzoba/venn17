...
def poleEncodedLinkCircle (p : Fin 2) : Circle ≃ₜ
    Coordinates.link (encodedCells suppliedModel.oriented 131072) (poleIndex p).val :=
  (supplied_dart_link_circle (poleIndex p)).trans
    (Homeomorph.setCongr (encoded_link_eq_dartLink suppliedModel.oriented 131072 _
      (triangleVertices_injective _ _ supplied_corners_bounded supplied_distinct_corners_verified)).symm)

def poleRotationAngle (p : Fin 2) : ℝ := if p = 0 then 2 * Real.pi / 17 else -(2 * Real.pi / 17)
...
theorem encodedRotation_fixes_pole (p : Fin 2) : encodedRotation (poleIndex p).val = (poleIndex p).val := by
  have hb : (poleIndex p).val < 131072 + suppliedModel.oriented.size := by
    simp only [poleIndex]
    split_ifs <;> omega
...
def poleDiskHomeomorph (p : Fin 2) : ClosedDisk ≃ₜ
    range (Coordinates.linkConeParameter (encodedCells suppliedModel.oriented 131072)
...
disk, including its center, with the generator angle ±2π/17. -/
...
theorem pole_disk_mem_realization (p : Fin 2) (z : ClosedDisk) :
    (poleDiskHomeomorph p z).val ∈ Coordinates.realization (encodedCells suppliedModel.oriented 131072) := by
...
theorem diagram_rotation_encoded (x : Diagram) :
  ...
  change Coordinates.reindex (Coordinates.labelEquiv 131072 suppliedModel.oriented.size)
      (Quad.coordinateHomeomorph diagramAutomorphism.labels x.val) =
    Coordinates.reindex encodedRotation
      (Coordinates.reindex (Coordinates.labelEquiv 131072 suppliedModel.oriented.size) x.val)
  ext j
...
def polePattern (p : Fin 2) : Fin 131072 := if p = 0 then 0 else 131071
...
theorem pole_disk_range_mem_nhds (p : Fin 2) :
  ...
  intro x hx
  have hf : ∀ t i, i ∈ encodedCells suppliedModel.oriented 131072 t →
      ((encodedCells suppliedModel.oriented 131072 t).erase i).Nonempty := by
    intro t i hi
    apply Finset.card_pos.mp
    rw [Finset.card_erase_of_mem hi, diagram_encoded_cell_card]
    decide
  have hh := Coordinates.openStar_eq_cone (encodedCells suppliedModel.oriented 131072)
  ...
  have hc : (diagramEncodedHomeomorph x).val ∈
    ...
    have hm : (diagramEncodedHomeomorph x).val ∈ Coordinates.openStar
        (encodedCells suppliedModel.oriented 131072) (poleIndex p).val :=
