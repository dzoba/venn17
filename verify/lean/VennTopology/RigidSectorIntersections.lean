...
theorem rigid_sector_angle_bounds (k : Fin 17) {θ : ℝ} (hθ : θ ∈ Ico 0 (2 * Real.pi))
    (hd : 0 ≤ planarCross (rigidDirection k) (Circle.exp θ))
    (he : planarCross (rigidDirection (cyclicNext k)) (Circle.exp θ) ≤ 0) :
    ((k.val : ℝ) * (2 * Real.pi / 17) ≤ θ ∧
      θ ≤ ((k.val : ℝ)+1) * (2 * Real.pi / 17)) ∨ (θ = 0 ∧ k.val = 16) := by
  rw [rigidDirection_angle, planarCross_exp] at hd
  rw [rigidDirection_next_angle, planarCross_exp] at he
  have hδ : 0 < 2 * Real.pi / 17 := by positivity
  have hk0 : 0 ≤ (k.val : ℝ) := Nat.cast_nonneg _
  have hk16 : (k.val : ℝ) ≤ 16 := by exact_mod_cast (show k.val ≤ 16 by omega)
  have ht : Real.sin ((θ-(k.val : ℝ)*(2*Real.pi/17))-(2*Real.pi/17)) ≤ 0 := by
    convert he using 2; ring
  obtain h | h := sine_sector_interval hδ
    (a := θ-(k.val : ℝ)*(2*Real.pi/17))
  ...
  · right
    ...
    refine ⟨hzero, ?_⟩
    have heq : (k.val : ℝ) = 16 := by nlinarith [hθ.1]
    exact_mod_cast heq

theorem rigidSector_polar_bounds (k : Fin 17) (x : SphereRepresentative)
...
theorem rigidSectorParam_range (k : Fin 17) :
...
theorem rigidSector_polar_endpoint (k : Fin 17) (x : SphereRepresentative)
    (r θ : ℝ) (hr : 0 ≤ r) (hp : horizontal x.val = (r : ℂ) * Circle.exp θ)
    (he : θ = (k.val : ℝ) * (2 * Real.pi / 17) ∨
      θ = ((k.val : ℝ)+1) * (2 * Real.pi / 17)) : x ∈ range (rigidSectorParam k) := by
...
theorem rigid_sector_inter_subset_boundary (k l : Fin 17) (hkl : k ≠ l) :
  ...
  · by_cases he : θ = (k.val : ℝ) * (2 * Real.pi / 17) ∨
        θ = ((k.val : ℝ)+1) * (2 * Real.pi / 17)
    · exact rigidSector_polar_endpoint k x _ _ hr.le hc he
    have hlow : (k.val : ℝ) * (2 * Real.pi / 17) < θ := lt_of_le_of_ne hk.1
      (fun h => he (Or.inl h.symm))
    have hupp : θ < ((k.val : ℝ)+1) * (2 * Real.pi / 17) := lt_of_le_of_ne hk.2
      (fun h => he (Or.inr h))
  ...
  · rw [rigidSectorParam_range]
    ...
    have hnext : cyclicNext k = (0 : Fin 17) := by
      apply Fin.ext
      simp [cyclicNext, hk]
    rw [hnext]
