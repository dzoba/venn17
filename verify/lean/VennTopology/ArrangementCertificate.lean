...
theorem realized_arrangement_verified :
    ...
    (∀ v : Fin 131072, patternPoint v ∈ diagramRegion v ∧ IsPathConnected (diagramRegion v)) ∧
    (∀ x : Diagram, x ∉ ⋃ i : Fin 17, diagramCurve i ↔ ∃! v : Fin 131072, x ∈ diagramRegion v) ∧
    Nat.card (ConnectedComponents diagramComplement) = 131072 ∧
    (∀ i j k : Fin 17, i ≠ j → i ≠ k → j ≠ k → ∀ x : Diagram,
      x ∈ diagramCurve i → x ∈ diagramCurve j → x ∉ diagramCurve k) ∧
    (∀ x : Diagram, (∃ i j : Fin 17, i ≠ j ∧ x ∈ diagramCurve i ∧ x ∈ diagramCurve j) ↔
      ∃ f : Fin suppliedModel.oriented.size, x = crossingPoint f) ∧
    (∀ f : Fin suppliedModel.oriented.size, ∃ i j : Fin 17, i ≠ j ∧ ∀ k : Fin 17,
    ...
    (∀ i : Fin 17, Nat.card (ConnectedComponents (diagramCurveComplement i)) = 2 ∧
      ∀ b : Bool, IsOpen (diagramSide i b) ∧ IsPathConnected (diagramSide i b)) ∧
    (∀ v : Fin 131072, diagramRegion v = ⋂ i : Fin 17, diagramSide i (v.val.testBit i.val)) :=
