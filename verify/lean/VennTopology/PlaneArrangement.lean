...
@[simp] theorem realizedPlaneCurve_mem (x : PuncturedDiagram) (i : Fin 17) :
...
theorem planar_no_triple_intersections (i j k : Fin 17)
...
theorem planar_crossing_iff (x : Plane) :
    (∃ i j : Fin 17, i ≠ j ∧ x ∈ realizedPlaneCurve i ∧ x ∈ realizedPlaneCurve j) ↔
...
theorem planar_symmetry_period (x : Plane) : realizedPlaneSymmetry^[17] x = x := by
  obtain ⟨y, rfl⟩ := diagramPlaneHomeomorph.surjective x
  rw [planar_symmetry_iterate, punctured_rotation_period]

theorem planar_symmetry_no_short_period (k : ℕ) (hk : 0 < k) (hk17 : k < 17) :
...
theorem planar_symmetry_curve_image (i : Fin 17) :
...
theorem planar_symmetry_region_image (v : Fin 131072) :
...
theorem planar_symmetry_fixed_iff (x : Plane) :
    realizedPlaneSymmetry x = x ↔ (diagramPlaneHomeomorph.symm x).val = patternPoint 131071 := by
  obtain ⟨y, rfl⟩ := diagramPlaneHomeomorph.surjective x
...
theorem planar_symmetry_unique_fixed_point : ∃! x : Plane, realizedPlaneSymmetry x = x := by
  let p : PuncturedDiagram := ⟨patternPoint 131071, fun h =>
    (by decide : (131071 : Fin 131072) ≠ 0) (patternPoint_injective h)⟩
