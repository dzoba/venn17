...
def parse (text : String) : Except String (Array Face) := do
  ...
  failUnless (n == 17) "expected 17 curves"
...
def makeModel (faces : Array Face) : Model := Id.run do
  ...
  let mut graph : Graph := Array.replicate (2^17) #[]
  let mut dual : Graph := Array.replicate faces.size #[]
  let mut constraints : Array (Array (Nat × Bool)) := Array.replicate faces.size #[]
  let mut curves : Array Graph := Array.replicate 17 (Array.replicate faces.size #[])
  ...
  let mut links : Array (Std.HashMap Nat Nat) := Array.replicate (2^17) {}
...
An arbitrary order-17 automorphism alone need not have rotation number ±1/17. -/
def poleRotationOneStep (m : Model) : Bool :=
  let atPole := fun pole step =>
    m.links[pole]!.size == 17 && (Array.range 17).all fun i =>
      m.links[pole]![pole ^^^ 2^i]? == some (pole ^^^ 2^((i+step)%17))
  (atPole 0 1 && atPole (2^17-1) 16) || (atPole 0 16 && atPole (2^17-1) 1)

def allPatterns (m : Model) : Bool := Id.run do
  let mut seen := Array.replicate (2^17) false
  for f in m.faces do
    for v in f do
...
def Report.Passed (r : Report) : Prop :=
  r.regions = 131072 ∧ r.arcs = 262140 ∧ r.crossings = 131070 ∧
...
def audit (text : String) : Except String Report := do
  ...
  return {
    ...
    poleRotationOneStep := Venn17.poleRotationOneStep m
  }
