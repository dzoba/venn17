...
class CheckerTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.original = json.loads((ROOT / 'venn19-closure-s196002.json').read_text())
    ...
    def run_case(self, data, expected, python=True):
        with tempfile.TemporaryDirectory() as d:
            ...
            if python:
                if expected:
                    self.assertEqual(check(path)['regions'], 524288)
                else:
    ...
    def test_repeated_corner(self):
        data = {'n': 19, 'faces': [copy.deepcopy(self.original['faces'][0])]}
        data['faces'][0][1] = data['faces'][0][0]
        self.run_case(data, False)

    def test_non_square(self):
        self.run_case({'n': 19, 'faces': [['0'*19, '1'*19, '01'+'0'*17, '10'+'0'*17]]}, False)

    def test_bad_pattern_and_size(self):
        for s in ['x'*19, '0'*18, '0'*20]:
            self.run_case({'n': 19, 'faces': [[s]*4]}, False)
        self.run_case({'n': 18, 'faces': []}, False)

    def test_empty(self):
        self.run_case({'n': 19, 'faces': []}, False)
