...
theorem sector_color_data (f : Fin suppliedModel.oriented.size) :
    suppliedSectorColors.getD f.val 19 < 19 ∧
    suppliedSectorColors.getD ((rotationWitnesses suppliedModel.oriented).getD f.val 0) 19 = (suppliedSectorColors.getD f.val 19 + 1) % 19 := by
...
def sectorColor (f : Fin suppliedModel.oriented.size) : Fin 19 :=
  ⟨suppliedSectorColors.getD f.val 19, (sector_color_data f).1⟩
...
theorem sectorColor_rotation (f : Fin suppliedModel.oriented.size) :
  ...
  change suppliedSectorColors.getD ((rotationWitnesses suppliedModel.oriented)[f.val]!) 19 = _
  simpa only [Array.getElem!_eq_getD, sectorColor, cyclicNext, show (default : Nat) = 0 from rfl] using h

def sectorSpace (k : Fin 19) : Set (Quad.Ambient Nat (Fin suppliedModel.oriented.size)) :=
  ⋃ f, ⋃ (_ : sectorColor f = k), ⋃ j, diagramQuad.triangle f j

def diagramSector (k : Fin 19) : Set Diagram := {x | x.val ∈ sectorSpace k}

def encodedSector (k : Fin 19) : Set (Point Nat) :=
  ⋃ t : Fin (4*suppliedModel.oriented.size), ⋃ (_ : sectorColor (triangleFace _ t) = k),
    simplex (encodedCells suppliedModel.oriented 524288 t)

theorem sectorSpace_subset_space (k : Fin 19) : sectorSpace k ⊆ diagramQuad.space := by
  intro x hx
  obtain ⟨f, _, j, hx⟩ := mem_iUnion.mp hx |>.imp (fun f => mem_iUnion₂.mp)
  exact diagramQuad.triangle_subset_space f j hx

theorem sectorSpace_compact (k : Fin 19) : IsCompact (sectorSpace k) :=
  isCompact_iUnion fun f => isCompact_iUnion fun _ => isCompact_iUnion fun j =>
    diagramQuad.triangle_compact f j

theorem diagramSector_closed (k : Fin 19) : IsClosed (diagramSector k) :=
  (sectorSpace_compact k).isClosed.preimage continuous_subtype_val

theorem diagramSector_compact (k : Fin 19) : IsCompact (diagramSector k) :=
...
theorem sector_encoded_image (k : Fin 19) :
    reindex (labelEquiv 524288 suppliedModel.oriented.size) '' sectorSpace k = encodedSector k := by
...
theorem mem_diagramSector_iff_encoded (k : Fin 19) (x : Diagram) :
    x ∈ diagramSector k ↔ (diagramEncodedHomeomorph x).val ∈ encodedSector k := by
  rw [← sector_encoded_image]
  change x.val ∈ sectorSpace k ↔ reindex (labelEquiv 524288 suppliedModel.oriented.size) x.val ∈ _
...
theorem sectorSpace_rotation (k : Fin 19) :
...
theorem diagram_sector_rotation (k : Fin 19) :
...
theorem sector_boundary_subset (k : Fin 19) : diagramMeridianBoundary k (cyclicNext k) ⊆ diagramSector k := by
  intro x hx
...
theorem diagramSector_nonempty (k : Fin 19) : (diagramSector k).Nonempty := by
