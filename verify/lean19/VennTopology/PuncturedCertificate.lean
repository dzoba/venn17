...
theorem punctured_arrangement_verified :
    (∀ i : Fin 19, ∃ f : Circle → PuncturedDiagram,
      Continuous f ∧ Function.Injective f ∧ Set.range f = puncturedCurve i) ∧
    (∀ v : Fin 524288, IsPathConnected (puncturedRegion v)) ∧
    (∀ x : PuncturedDiagram, x ∈ puncturedComplement ↔ ∃! v : Fin 524288, x ∈ puncturedRegion v) ∧
    Nat.card (ConnectedComponents puncturedComplement) = 524288 ∧
    (∀ i j k : Fin 19, i ≠ j → i ≠ k → j ≠ k → ∀ x : PuncturedDiagram,
    ...
    (∀ i : Fin 19, puncturedRotation '' puncturedCurve i = puncturedCurve (curveRotation i)) ∧
    (∀ v : Fin 524288, puncturedRotation '' puncturedRegion v = puncturedRegion (patternRotation v)) ∧
    (∀ x : PuncturedDiagram, puncturedRotation^[19] x = x) ∧
    (∀ k : Nat, 0 < k → k < 19 → ∃ x : PuncturedDiagram, puncturedRotation^[k] x ≠ x) ∧
    (∀ x : PuncturedDiagram, puncturedRotation x = x ↔ x.val = patternPoint 524287) ∧
    ...
    (∀ i : Fin 19, Nat.card (ConnectedComponents (puncturedCurveComplement i)) = 2 ∧
      ∀ b : Bool, IsOpen (puncturedSide i b) ∧ IsPathConnected (puncturedSide i b)) ∧
    (∀ v : Fin 524288, puncturedRegion v = ⋂ i : Fin 19, puncturedSide i (v.val.testBit i.val)) :=
