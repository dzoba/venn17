...
def rigidPlaneCurve (i : Fin 19) : Set Plane := planeCurve rigidPlaneHomeomorph i
def rigidPlaneSide (i : Fin 19) (b : Bool) : Set Plane := planeSide rigidPlaneHomeomorph i b
def rigidPlaneRegion (v : Fin 524288) : Set Plane := planeRegion rigidPlaneHomeomorph v
def rigidPlaneCrossing (f : Fin suppliedModel.oriented.size) : Plane :=
  rigidPlaneHomeomorph (puncturedCrossingPoint f)

theorem rigidPlaneCurve_mem (x : PuncturedDiagram) (i : Fin 19) :
    rigidPlaneHomeomorph x ∈ rigidPlaneCurve i ↔ x ∈ puncturedCurve i :=
  rigidPlaneHomeomorph.injective.mem_set_image

theorem rigidPlane_curve_rotation (i : Fin 19) :
...
theorem rigidPlane_region_rotation (v : Fin 524288) :
...
theorem rigidPlane_membership_verified :
    (∀ v : Fin 524288, IsPathConnected (rigidPlaneRegion v) ∧
      rigidPlaneRegion v = ⋂ i : Fin 19, rigidPlaneSide i (v.val.testBit i.val)) ∧
    (∀ x : Plane, x ∉ ⋃ i : Fin 19, rigidPlaneCurve i ↔
      ∃! v : Fin 524288, x ∈ rigidPlaneRegion v) :=
...
theorem rigidPlaneRotation_period (x : Plane) : rigidPlaneRotation^[19] x = x := by
  obtain ⟨y, rfl⟩ := rigidPlaneHomeomorph.surjective x
  rw [rigidPlaneRotation_iterate, punctured_rotation_period]

theorem rigidPlaneRotation_no_short_period (k : ℕ) (hk : 0 < k) (hk17 : k < 19) :
...
theorem rigidPlane_no_triple (i j k : Fin 19) (hij : i ≠ j) (hik : i ≠ k) (hjk : j ≠ k)
...
theorem rigidPlane_crossing_iff (x : Plane) :
    (∃ i j : Fin 19, i ≠ j ∧ x ∈ rigidPlaneCurve i ∧ x ∈ rigidPlaneCurve j) ↔
...
def vennIndexOrder : Equiv.Perm (Fin 19) := Equiv.neg _

theorem vennIndexOrder_next (i : Fin 19) :
    curveRotation (vennIndexOrder i) = vennIndexOrder (cyclicNext i) := by
  decide +revert

def rotationalVennCurve (i : Fin 19) : Set Plane := rigidPlaneCurve (vennIndexOrder i)
def rotationalVennSide (i : Fin 19) (b : Bool) : Set Plane := rigidPlaneSide (vennIndexOrder i) b

theorem rotationalVenn_rotation (i : Fin 19) :
...
theorem rotationalVenn_all_patterns (b : Fin 19 → Bool) :
...
angle `2π/19` about zero. `IsPathConnected` includes nonemptiness. -/
structure SimpleRotationalVenn (C : Fin 19 → Set Plane) (S : Fin 19 → Bool → Set Plane) : Prop where
  ...
  regions : ∀ b : Fin 19 → Bool, IsPathConnected (⋂ i, S i (b i))
...
theorem exists_simple_rotational_venn_19 :
    ∃ (C : Fin 19 → Set Plane) (S : Fin 19 → Bool → Set Plane), SimpleRotationalVenn C S :=
