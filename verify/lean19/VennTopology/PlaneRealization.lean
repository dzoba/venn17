...
not yet identify the induced order-19 homeomorphism with a rigid rotation. -/
...
def realizedPlaneCurve (i : Fin 19) : Set Plane := planeCurve diagramPlaneHomeomorph i
def realizedPlaneSide (i : Fin 19) (b : Bool) : Set Plane := planeSide diagramPlaneHomeomorph i b
def realizedPlaneRegion (v : Fin 524288) : Set Plane := planeRegion diagramPlaneHomeomorph v

/-- Actual planar Jordan curves with all 2^19 nonempty connected membership
...
theorem planar_venn_diagram_verified :
    ...
    (∀ v : Fin 524288, IsPathConnected (realizedPlaneRegion v) ∧
      realizedPlaneRegion v = ⋂ i : Fin 19, realizedPlaneSide i (v.val.testBit i.val)) ∧
    (∀ x : Plane, x ∉ ⋃ i : Fin 19, realizedPlaneCurve i ↔
      ∃! v : Fin 524288, x ∈ realizedPlaneRegion v) ∧
    (∀ b : Fin 19 → Bool, IsPathConnected (⋂ i : Fin 19, realizedPlaneSide i (b i))) :=
