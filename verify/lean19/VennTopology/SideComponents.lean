...
theorem diagramSide_isOpen (i : Fin 19) (b : Bool) : IsOpen (diagramSide i b) :=
  isOpen_lt continuous_const (continuous_const.mul (diagramBitField_continuous i))

theorem diagramSide_subset_complement (i : Fin 19) (b : Bool) :
...
theorem diagramSide_disjoint (i : Fin 19) : Disjoint (diagramSide i false) (diagramSide i true) := by
...
theorem diagramSide_cover (i : Fin 19) : (⋃ b : Bool, diagramSide i b) = (diagramCurve i)ᶜ := by
  ext x
  constructor
...
def diagramCurveComplement (i : Fin 19) : Set Diagram := (diagramCurve i)ᶜ

def curveComplementSide (i : Fin 19) (b : Bool) : Set (diagramCurveComplement i) :=
  Subtype.val ⁻¹' diagramSide i b

theorem curveComplementSide_isOpen (i : Fin 19) (b : Bool) : IsOpen (curveComplementSide i b) :=
  (diagramSide_isOpen i b).preimage continuous_subtype_val

theorem curveComplementSide_pairwise (i : Fin 19) :
...
theorem curveComplementSide_cover (i : Fin 19) : (⋃ b, curveComplementSide i b) = univ := by
...
theorem curveComplementSide_isClopen (i : Fin 19) (b : Bool) : IsClopen (curveComplementSide i b) := by
...
theorem curveComplementSide_pathConnected (i : Fin 19) (b : Bool) :
...
def diagramSideComponentsEquiv (i : Fin 19) : ConnectedComponents (diagramCurveComplement i) ≃ Bool :=
...
theorem diagram_curve_complement_component_count (i : Fin 19) :
    Nat.card (ConnectedComponents (diagramCurveComplement i)) = 2 := by
  rw [Nat.card_congr (diagramSideComponentsEquiv i)]
  simp

theorem diagram_region_side_iff (i : Fin 19) (v : Fin 524288) (b : Bool)
...
theorem pattern_eq_of_bits {u v : Fin 524288}
    (h : ∀ i : Fin 19, u.val.testBit i.val = v.val.testBit i.val) : u = v := by
  apply Fin.ext
  apply Nat.eq_of_testBit_eq
  intro i
  by_cases hi : i < 19
  · exact h ⟨i,hi⟩
  · have hp : 524288 ≤ 2^i := by
      calc
        524288 = 2^19 := by norm_num
...
/-- Regions are exactly the intersections specified by their 19 geometric
side choices, not merely sets containing correctly labelled vertices. -/
theorem diagramRegion_eq_sides (v : Fin 524288) :
    diagramRegion v = ⋂ i : Fin 19, diagramSide i (v.val.testBit i.val) := by
  ext x
  constructor
  · intro hx
  ...
  · intro hx
    ...
    have hc : x ∉ ⋃ i : Fin 19, diagramCurve i := by
      intro hc
...
theorem diagram_sides_verified (i : Fin 19) :
    ...
    (∀ v : Fin 524288, ∀ b : Bool, ∀ x ∈ diagramRegion v,
