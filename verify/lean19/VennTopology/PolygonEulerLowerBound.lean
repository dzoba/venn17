...
theorem diagram_finite_vertices_covered (v : diagramFiniteTriangulation.Vertex) :
  ...
  refine ⟨⟨Coordinates.FiniteCoordinates.cell (524288 + suppliedModel.oriented.size)
    (encodedCells suppliedModel.oriented 524288 t), ?_⟩, ?_⟩
  · exact Finset.mem_image.mpr ⟨t, Finset.mem_univ _, rfl⟩
  · exact Coordinates.FiniteCoordinates.mem_cell.mpr ht

theorem diagram_polygon_vertex_count_lower_bound : 1048574 ≤ Nat.card (Vertex diagramPresentation) := by
