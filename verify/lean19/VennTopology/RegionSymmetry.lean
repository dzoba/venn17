...
theorem regionStep_lt {v : Nat} (hv : v < 524288) : regionStep v < 524288 := by
...
def patternRotation : Equiv.Perm (Fin 524288) :=
  Fin.equivSubtype.trans ((regionPermutation.subtypeEquiv
    (p := fun v => v < 524288) (q := fun v => v < 524288) (by
    intro v
    change v < 524288 ↔ regionStep v < 524288
    constructor
    · exact regionStep_lt
    · intro h
      by_contra hv
...
@[simp] theorem patternRotation_val (v : Fin 524288) :
    (patternRotation v).val = regionStep v.val := rfl

theorem diagram_encoded_region_coordinate (x : Diagram) (v : Fin 524288) :
    (diagramEncodedHomeomorph x).val v.val = x.val (.inl v.val) := by
  change Coordinates.reindex (Coordinates.labelEquiv 524288 suppliedModel.oriented.size) x.val v.val = _
  rw [Coordinates.reindex_apply]
  apply congrArg x.val
  exact (Coordinates.labelEquiv 524288 suppliedModel.oriented.size).symm_apply_eq.mpr
    (Coordinates.labelEquiv_region _ _ _ v.isLt).symm

theorem diagram_rotation_region_coordinate (x : Diagram) (v : Fin 524288) :
...
theorem diagram_rotation_mem_region (x : Diagram) (v : Fin 524288) :
...
theorem diagram_rotation_region_image (v : Fin 524288) :
