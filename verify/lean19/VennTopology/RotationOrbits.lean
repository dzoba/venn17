...
orbit quotient is a genuine 19-sheeted covering, as needed for a construction
...
theorem planar_minimal_period (x : Plane) (hx : realizedPlaneSymmetry x ≠ x) :
    minimalPeriod realizedPlaneSymmetry x = 19 :=
...
theorem planar_orbit_injective (x : Plane) (hx : realizedPlaneSymmetry x ≠ x) :
    Function.Injective (fun k : Fin 19 => realizedPlaneSymmetry^[k.val] x) := by
  intro i j hij
  apply Fin.ext
...
instance : MulAction RotationGroup FreePlane where
  ...
  mul_smul g h x := by
    ...
    rw [ZMod.val_add,
      (show IsPeriodicPt freePlaneSymmetry 19 x from freePlaneSymmetry_period x).iterate_mod_apply,
...
instance : IsCancelSMul RotationGroup FreePlane where
  right_cancel' g h x he := by
    ...
    exact ZMod.val_injective 19 (congrArg Fin.val hv)
...
theorem rotationOrbitProjection_fiber_card (q : RotationOrbitSpace) :
    Nat.card (rotationOrbitProjection ⁻¹' {q}) = 19 := by
