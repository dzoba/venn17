...
theorem rigidDirection_angle (k : Fin 19) :
    rigidDirection k = Circle.exp ((k.val : ℝ) * (2 * Real.pi / 19)) := by
  rw [Circle.exp_natCast_mul]
  rfl

theorem rigidDirection_next_angle (k : Fin 19) :
    rigidDirection (cyclicNext k) = Circle.exp (((k.val : ℝ)+1) * (2 * Real.pi / 19)) := by
...
theorem angle_in_rigid_sector {θ : ℝ} (hθ : θ ∈ Ico 0 (2 * Real.pi)) :
    ∃ k : Fin 19, (k.val : ℝ) * (2 * Real.pi / 19) ≤ θ ∧
      θ ≤ ((k.val : ℝ)+1) * (2 * Real.pi / 19) := by
  let a : ℝ := 2 * Real.pi / 19
...
theorem circle_in_rigid_sector (c : Circle) :
    ∃ k : Fin 19, 0 ≤ planarCross (rigidDirection k) c ∧
