...
def equivOfPeriod {X : Type*} (f : X → X) (h : ∀ x, f^[19] x = x) : Equiv.Perm X where
  toFun := f
  invFun := f^[18]
...
theorem region_period (v : Nat) : regionStep^[19] v = v := by
  by_cases hv : v < 2^19
...
theorem face_check_at (fs : Array Face) (h : faceRotationCheck fs = true)
    ...
    steps (fun f => (rotationWitnesses fs)[f]!) 19 f = f ∧
