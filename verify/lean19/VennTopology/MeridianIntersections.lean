...
theorem meridianLabel_injective (k : Fin 19) : Function.Injective (meridianLabel k) := by
  intro i j h
  have hi := meridian_labels_verified.2.2 k (cyclicNext k) (cyclicNext_ne (by decide) k).symm
  have he : (⟨i.val, by omega⟩ : Fin 38) = ⟨j.val, by omega⟩ := hi (by
    simpa only [meridian_boundary_front] using h)
  have hv := congrArg Fin.val he
  exact Fin.ext hv

/-- Different meridians have only the two endpoint labels in common. -/
theorem meridianLabel_shared (k l : Fin 19) (hkl : k ≠ l) (i j : Fin 20)
    (h : meridianLabel k i = meridianLabel l j) :
    (i = 0 ∧ j = 0) ∨ (i = 19 ∧ j = 19) := by
  have hz (k : Fin 19) := (meridian_labels_verified.1 k).1
  have hn (k : Fin 19) := (meridian_labels_verified.1 k).2
  by_cases hi0 : i = 0
  · left
  ...
  · have he : i = 0 := meridianLabel_injective k (by simpa [hj0, hz] using h)
    exact (hi0 he).elim
  by_cases hj17 : j = 19
  · have he : i = 19 := meridianLabel_injective k (by simpa [hj17, hn] using h)
    exact (hi17 he).elim
  have hi : 0 < i.val ∧ i.val < 19 := by
    have : i.val ≠ 0 ∧ i.val ≠ 19 := ⟨fun he => hi0 (Fin.ext he), fun he => hi17 (Fin.ext he)⟩
    omega
  have hj : 0 < j.val ∧ j.val < 19 := by
    have : j.val ≠ 0 ∧ j.val ≠ 19 := ⟨fun he => hj0 (Fin.ext he), fun he => hj17 (Fin.ext he)⟩
    omega
  have he : (⟨i.val, by omega⟩ : Fin 38) = ⟨38-j.val, by omega⟩ :=
    meridian_labels_verified.2.2 k l hkl (by
      simpa [meridianBoundaryLabel, show i.val < 20 from i.isLt,
        show ¬38-j.val < 20 by omega, show 38-(38-j.val) = j.val by omega] using h)
  have hv := congrArg Fin.val he
  change i.val = 38 - j.val at hv
  omega

private theorem meridian_edge_shared (k l : Fin 19) (hkl : k ≠ l) (i j : Fin 19)
    (a : Nat)
    ...
    a = if i.val = 0 then 0 else 524287 := by
  rcases ha with rfl | rfl
  · have he : (⟨i.val, by omega⟩ : Fin 20) = 0 := by
      rcases hb with hb | hb <;>
        rcases meridianLabel_shared k l hkl _ _ hb with h | h
      · exact h.1
      · have hv := congrArg Fin.val h.1; change i.val = 19 at hv; omega
      · exact h.1
      · have hv := congrArg Fin.val h.1; change i.val = 19 at hv; omega
    have hi : i.val = 0 := congrArg Fin.val he
    rw [he, if_pos hi, (meridian_labels_verified.1 k).1]
  · have he : (⟨i.val+1, by omega⟩ : Fin 20) = 19 := by
      rcases hb with hb | hb <;>
        rcases meridianLabel_shared k l hkl _ _ hb with h | h
...
theorem encoded_meridians_inter_subset (k l : Fin 19) (hkl : k ≠ l) :
    encodedMeridian k ∩ encodedMeridian l ⊆ {vertex 0, vertex 524287} := by
  rintro x ⟨hk, hl⟩
  ...
  have hx := simplex_inter_subset_vertex _ _ (if i.val = 0 then 0 else 524287)
...
theorem diagram_meridians_inter (k l : Fin 19) (hkl : k ≠ l) :
    diagramMeridian k ∩ diagramMeridian l = {patternPoint 0, patternPoint 524287} := by
  ...
  · intro x hx
    ...
    · right
      ...
      simpa [patternPoint_encoded] using he
  · intro x hx
    have hz (m : Fin 19) : patternPoint 0 ∈ diagramMeridian m := by
      rw [← diagramMeridianMap_range]
      exact ⟨⟨0, by norm_num⟩, diagramMeridianMap_zero m⟩
    have hn (m : Fin 19) : patternPoint 524287 ∈ diagramMeridian m := by
      rw [← diagramMeridianMap_range]
      exact ⟨⟨19, by norm_num⟩, diagramMeridianMap_one m⟩
...
theorem meridian_boundary_back (k l : Fin 19) (i : Fin 20) :
    meridianBoundaryLabel k l ⟨(38-i.val)%38, Nat.mod_lt _ (by decide)⟩ =
      meridianLabel l i := by
  fin_cases i <;> simp [meridianBoundaryLabel, meridian_labels_verified.1]

private theorem meridian_boundary_first_edge (k l : Fin 19) (i : Fin 19) :
...
private theorem meridian_boundary_second_edge (k l : Fin 19) (i : Fin 19) :
    segment ℝ (vertex (meridianBoundaryLabel k l ⟨37-i.val, by omega⟩))
      (vertex (meridianBoundaryLabel k l (cyclicNext ⟨37-i.val, by omega⟩))) =
  ...
  rw [h2, h1, meridian_boundary_back k l (⟨i.val+1, by omega⟩ : Fin 20),
    meridian_boundary_back k l (⟨i.val, by omega⟩ : Fin 20), segment_symm]

theorem meridian_boundary_eq_union (k l : Fin 19) :
  ...
  · intro hx
    ...
    · right
      let i : Fin 19 := ⟨37-j.val, by omega⟩
      have he : (⟨37-i.val, by omega⟩ : Fin 38) = j := by
  ...
  · rintro (hx | hx)
    ...
    · obtain ⟨i, hi⟩ := mem_iUnion.mp hx
      exact mem_iUnion.mpr ⟨⟨37-i.val, by omega⟩, (meridian_boundary_second_edge k l i).symm ▸ hi⟩

theorem diagram_meridian_boundary_eq_union (k l : Fin 19) :
...
theorem diagram_meridian_system :
    (∀ k, Continuous (diagramMeridianMap k) ∧ Function.Injective (diagramMeridianMap k) ∧
      ...
      diagramMeridianMap k ⟨19, by norm_num⟩ = patternPoint 524287) ∧
    (∀ k l, k ≠ l → diagramMeridian k ∩ diagramMeridian l =
      {patternPoint 0, patternPoint 524287}) ∧
