...
/-- The 19 equally spaced meridian directions. -/
def rigidDirection (k : Fin 19) : Circle := rigidGenerator ^ k.val

theorem rigidDirection_next (k : Fin 19) :
    rigidDirection (cyclicNext k) = rigidGenerator * rigidDirection k := by
  change rigidGenerator ^ ((k.val+1)%19) = rigidGenerator * rigidGenerator^k.val
  by_cases h : k.val + 1 < 19
  · rw [Nat.mod_eq_of_lt h, pow_succ']
  · have hk : k.val = 18 := by omega
    simp [hk, ← pow_succ', rigidGenerator_pow_nineteen]

theorem rigidDirection_injective : Function.Injective rigidDirection := by
  intro k l h
  have ha : 0 < 2 * Real.pi / 19 := by positivity
  have hm (i : Fin 19) : (i.val : ℝ) * (2 * Real.pi / 19) ∈ Ico 0 (2 * Real.pi) := by
    have hi : (i.val : ℝ) < 19 := by exact_mod_cast i.isLt
    constructor
    · positivity
    · nlinarith
  have he : Circle.exp ((k.val : ℝ) * (2 * Real.pi / 19)) =
      Circle.exp ((l.val : ℝ) * (2 * Real.pi / 19)) := by
...
theorem rigidDirection_ne_next (k : Fin 19) : rigidDirection k ≠ rigidDirection (cyclicNext k) := by
  intro h
  have hval := congrArg Fin.val (rigidDirection_injective h)
  change k.val = (k.val+1)%19 at hval
  omega

/-- The prescribed boundary for target sector `k`. -/
def rigidSectorBoundary (k : Fin 19) : Circle → SphereRepresentative :=
  sphereMeridianBoundary (rigidDirection k) (rigidDirection (cyclicNext k))

theorem rigidSectorBoundary_continuous (k : Fin 19) : Continuous (rigidSectorBoundary k) :=
  sphereMeridianBoundary_continuous _ _

theorem rigidSectorBoundary_injective (k : Fin 19) : Function.Injective (rigidSectorBoundary k) :=
  sphereMeridianBoundary_injective (rigidDirection_ne_next k)

theorem rigidSectorBoundary_rotation (k : Fin 19) (z : Circle) :
