...
def sectorColorCheck (fs : Array Face) (cs : Array Nat) : Bool :=
  ...
  cs.size == fs.size && (Array.range fs.size).all fun f =>
    cs.getD f 19 < 19 && cs.getD (rotations.getD f 0) 19 == (cs.getD f 19 + 1) % 19

def sectorVertexMasks (fs : Array Face) (cs : Array Nat) : Array Nat := Id.run do
  let mut masks := Array.replicate 524288 0
  for f in [:fs.size] do
    for v in fs[f]! do
      masks := masks.modify v (fun m => m ||| 2^(cs.getD f 19))
  return masks
...
def sectorCornerCheck (fs : Array Face) (cs masks : Array Nat) : Bool :=
  (Array.range fs.size).all fun f => (Array.range 4).all fun i =>
    ...
    v < 524288 && (masks.getD v 0).testBit (cs.getD f 19)

def sectorBoundaryTriangle (fs : Array Face) (k : Fin 19) (j : Fin 38) : Nat :=
...
def sectorBoundaryRows : Array (Array Nat) := Array.ofFn fun k : Fin 19 =>
...
38 boundary edges of its own sector. This is only a finite support statement. -/
def sectorOverlapCheck (fs : Array Face) (cs masks : Array Nat) : Bool :=
  ...
  (Array.range fs.size).all fun f => (Array.range 4).all fun i =>
    ...
    let k := cs.getD f 19
    let row := rows.getD k #[]
    (Array.range 19).all fun l => l == k || (Array.range 38).any fun j =>
      let a := row.getD j 0
      let b := row.getD ((j+1)%38) 0
...
theorem sector_boundary_verified : ∀ k j,
    ...
    let a := meridianBoundaryLabel k (Coordinates.cyclicNext k) j
    let b := meridianBoundaryLabel k (Coordinates.cyclicNext k) (Coordinates.cyclicNext j)
    t < 4 * suppliedModel.oriented.size ∧ suppliedSectorColors.getD (t/4) 19 = k.val ∧
...
theorem sector_connected_verified : ∀ k : Fin 19,
    GraphProof.connectedCheck suppliedModel.dual
      (fun f => suppliedSectorColors.getD f 19 == k.val) = true := by
  native_decide

theorem sector_face_counts_verified : ∀ k : Fin 19,
    (suppliedSectorColors.filter (· == k.val)).size = 27594 := by
  native_decide
