...
def sectorBoundaryParam (k : Fin 19) : UnitCircle → Diagram :=
  diagramMeridianBoundaryMap k (Coordinates.cyclicNext k) ∘ JordanCurve.Arcs.spherePlaneHomeoCircle

theorem sectorBoundaryParam_continuous (k : Fin 19) : Continuous (sectorBoundaryParam k) :=
  (diagramMeridianBoundaryMap_continuous _ _).comp JordanCurve.Arcs.spherePlaneHomeoCircle.continuous

theorem sectorBoundaryParam_injective (k : Fin 19) : Function.Injective (sectorBoundaryParam k) :=
...
theorem sectorBoundaryParam_range (k : Fin 19) :
...
theorem sectorBoundaryParam_subset (k : Fin 19) : range (sectorBoundaryParam k) ⊆ diagramSector k := by
  rw [sectorBoundaryParam_range]
  exact sector_boundary_subset k

theorem sectorBoundaryParam_frontier (k : Fin 19) : frontier (diagramSector k) ⊆ range (sectorBoundaryParam k) := by
  rw [sectorBoundaryParam_range]
  exact sector_frontier_subset k

theorem sector_interior_witness (k : Fin 19) :
...
def sectorDiskHomeomorph (k : Fin 19) : diagramSector k ≃ₜ PlaneClosedDisk :=
...
theorem sectorDiskHomeomorph_boundary (k : Fin 19) (z : UnitCircle) :
...
theorem diagram_sectors_are_closed_disks :
    ∀ k : Fin 19, Nonempty (diagramSector k ≃ₜ PlaneClosedDisk) :=
