...
theorem supplied_link_rows :
    suppliedLinkStars.size = 524288 + suppliedModel.oriented.size ∧
    ∀ v : Fin suppliedLinkStars.size,
      LinkProof.rowCheck suppliedModel.oriented 524288 v.val
...
theorem every_link_has_cyclic_order (v : Fin suppliedLinkStars.size) :
    ...
    3 ≤ row.size ∧
    Function.Injective (fun k : Fin row.size => LinkProof.tail suppliedModel.oriented 524288
      (row.getD k.val 0)) ∧
    ∀ k : Fin row.size,
      row.getD k.val 0 < 12*suppliedModel.oriented.size ∧
      LinkProof.owner suppliedModel.oriented 524288 (row.getD k.val 0) = v.val ∧
      LinkProof.head suppliedModel.oriented 524288 (row.getD k.val 0) =
        LinkProof.tail suppliedModel.oriented 524288 (row.getD ((k.val+1)%row.size) 0) :=
...
theorem every_triangle_dart_occurs (d : Fin (12*suppliedModel.oriented.size)) :
    LinkProof.owner suppliedModel.oriented 524288 d.val < suppliedLinkStars.size ∧
    ∃ k : Fin (suppliedLinkStars.getD (LinkProof.owner suppliedModel.oriented 524288 d.val) #[]).size,
      (suppliedLinkStars.getD (LinkProof.owner suppliedModel.oriented 524288 d.val) #[]).getD k.val 0 =
        d.val :=
