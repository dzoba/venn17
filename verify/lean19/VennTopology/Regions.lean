...
def diagramRegion (v : Fin 524288) : Set Diagram :=
  {x | Coordinates.Dominates 524288 v.val (diagramEncodedHomeomorph x).val}

theorem diagram_complement_unique_region (x : Diagram) :
    x ∉ ⋃ i : Fin 19, diagramCurve i ↔ ∃! v : Fin 524288, x ∈ diagramRegion v := by
  have h := curve_complement_unique_region suppliedModel.oriented 524288 19
...
theorem diagramRegion_disjoint {u v : Fin 524288} (huv : u ≠ v) :
    Disjoint (diagramRegion u) (diagramRegion v) := by
  rw [Set.disjoint_left]
  intro x hxu hxv
  exact huv (Fin.ext (Coordinates.dominates_disjoint u.isLt v.isLt hxu hxv))

theorem supplied_region_coverage : coverageCheck suppliedModel.oriented 524288 = true := by
...
theorem patternPoint_mem_region (v : Fin 524288) : patternPoint v ∈ diagramRegion v := by
  change Coordinates.Dominates 524288 v.val
    (Coordinates.reindex (Coordinates.labelEquiv 524288 suppliedModel.oriented.size)
...
theorem diagramRegion_pathConnected (v : Fin 524288) : IsPathConnected (diagramRegion v) := by
  have hp := encoded_region_pathConnected suppliedModel.oriented 524288 supplied_region_coverage v
  have hp' := hp.preimage_coe (fun _ hx => hx.1)
  have he : (((↑) : Coordinates.realization (encodedCells suppliedModel.oriented 524288) →
      Coordinates.Point Nat) ⁻¹' Coordinates.region (encodedCells suppliedModel.oriented 524288) 524288 v.val) =
      {x | Coordinates.Dominates 524288 v.val x.val} := by
    ext x
...
theorem diagramRegion_isOpen (v : Fin 524288) : IsOpen (diagramRegion v) :=
  (dominates_isOpen 524288 v.val).preimage
    (continuous_subtype_val.comp diagramEncodedHomeomorph.continuous)

def diagramComplement : Set Diagram := (⋃ i : Fin 19, diagramCurve i)ᶜ

def complementRegion (v : Fin 524288) : Set diagramComplement :=
  Subtype.val ⁻¹' diagramRegion v

theorem region_subset_complement (v : Fin 524288) : diagramRegion v ⊆ diagramComplement := by
  intro x hx
...
theorem complementRegion_isOpen (v : Fin 524288) : IsOpen (complementRegion v) :=
...
theorem complementRegion_isClopen (v : Fin 524288) : IsClopen (complementRegion v) := by
  have he : (complementRegion v)ᶜ = ⋃ w : {w : Fin 524288 // w ≠ v}, complementRegion w.val := by
    ext x
    constructor
    · intro hx
...
theorem complementRegion_pathConnected (v : Fin 524288) : IsPathConnected (complementRegion v) :=
...
def diagramComponentsEquiv : ConnectedComponents diagramComplement ≃ Fin 524288 :=
...
theorem diagram_complement_component_count : Nat.card (ConnectedComponents diagramComplement) = 524288 := by
