...
theorem zero_region_subset_false_side (i : Fin 19) : diagramRegion 0 ⊆ diagramSide i false := by
  intro x hx
  exact (diagram_region_side_iff i 0 false hx).mpr (by simp)

theorem pole_not_true_side (i : Fin 19) : patternPoint 0 ∉ diagramSide i true := by
  intro hx
  have h := (diagram_region_side_iff i 0 true (patternPoint_mem_region 0)).mp hx
  simpa using h

theorem diagramSide_punctured_pathConnected (i : Fin 19) (b : Bool) :
...
def puncturedSide (i : Fin 19) (b : Bool) : Set PuncturedDiagram := Subtype.val ⁻¹' diagramSide i b

theorem puncturedSide_isOpen (i : Fin 19) (b : Bool) : IsOpen (puncturedSide i b) :=
  (diagramSide_isOpen i b).preimage continuous_subtype_val

theorem puncturedSide_pathConnected (i : Fin 19) (b : Bool) : IsPathConnected (puncturedSide i b) := by
...
theorem puncturedSide_cover (i : Fin 19) : (⋃ b, puncturedSide i b) = (puncturedCurve i)ᶜ := by
  change (⋃ b, Subtype.val ⁻¹' diagramSide i b) = (Subtype.val ⁻¹' diagramCurve i)ᶜ
  rw [← preimage_iUnion,diagramSide_cover,preimage_compl]

theorem puncturedSide_disjoint (i : Fin 19) : Disjoint (puncturedSide i false) (puncturedSide i true) :=
  (diagramSide_disjoint i).preimage Subtype.val

theorem puncturedRegion_eq_sides (v : Fin 524288) :
    puncturedRegion v = ⋂ i : Fin 19, puncturedSide i (v.val.testBit i.val) := by
  change Subtype.val ⁻¹' diagramRegion v = ⋂ i : Fin 19, Subtype.val ⁻¹' diagramSide i (v.val.testBit i.val)
  rw [diagramRegion_eq_sides,preimage_iInter]

def puncturedCurveComplement (i : Fin 19) : Set PuncturedDiagram := (puncturedCurve i)ᶜ

def puncturedCurveComplementSide (i : Fin 19) (b : Bool) : Set (puncturedCurveComplement i) :=
  Subtype.val ⁻¹' puncturedSide i b

def curveComplementForget (i : Fin 19) (x : puncturedCurveComplement i) : diagramCurveComplement i :=
  ⟨x.val.val,x.property⟩

theorem puncturedCurveComplementSide_isClopen (i : Fin 19) (b : Bool) :
...
theorem puncturedCurveComplementSide_pathConnected (i : Fin 19) (b : Bool) :
...
def puncturedSideComponentsEquiv (i : Fin 19) : ConnectedComponents (puncturedCurveComplement i) ≃ Bool :=
...
theorem punctured_curve_complement_component_count (i : Fin 19) :
