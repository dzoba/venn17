...
/-! The 19 standard spherical sectors and equivariant disk coordinates. -/
...
theorem rigidGenerator_cross_pos : 0 < planarCross (1 : ℂ) rigidGenerator := by
  have ha : 0 < 2 * Real.pi / 19 := by positivity
  have hb : 2 * Real.pi / 19 < Real.pi := by linarith [Real.pi_pos]
  simpa [planarCross, rigidGenerator, Circle.coe_exp, Complex.exp_im] using
    Real.sin_pos_of_pos_of_lt_pi ha hb

theorem rigidDirection_cross_pos (k : Fin 19) :
...
def rigidSector (k : Fin 19) : Set SphereRepresentative :=
  sphereLune (rigidDirection k) (rigidDirection (cyclicNext k))

theorem rigidSector_closed (k : Fin 19) : IsClosed (rigidSector k) := sphereLune_closed _ _
theorem rigidSector_compact (k : Fin 19) : IsCompact (rigidSector k) := sphereLune_compact _ _

def rigidSectorParam (k : Fin 19) : UnitCircle → SphereRepresentative :=
  sphereLuneParam (rigidDirection k) (rigidDirection (cyclicNext k))

theorem rigidSectorParam_subset (k : Fin 19) : range (rigidSectorParam k) ⊆ rigidSector k :=
  sphereLuneParam_subset _ _ (rigidDirection_cross_pos k)

theorem rigidSectorParam_rotation (k : Fin 19) (z : UnitCircle) :
    rigidSphereRotation rigidGenerator (rigidSectorParam k z) = rigidSectorParam (cyclicNext k) z :=
  rigidSectorBoundary_rotation k _

def rigidSectorDisk (k : Fin 19) : rigidSector k ≃ₜ PlaneClosedDisk :=
  sphereLuneDisk _ _ (rigidDirection_cross_pos k)

theorem rigidSectorDisk_boundary (k : Fin 19) (z : UnitCircle) :
...
theorem rigidSector_rotation (k : Fin 19) :
...
theorem rigidSector_from_zero (k : Fin 19) :
...
def equivariantRigidSectorDisk (k : Fin 19) : rigidSector k ≃ₜ PlaneClosedDisk :=
  (rigidSectorFromZero k).symm.trans (rigidSectorDisk 0)

theorem equivariantRigidSectorDisk_boundary (k : Fin 19) (z : UnitCircle) :
...
def rigidSectorRotatePoint (k : Fin 19) (x : rigidSector k) : rigidSector (cyclicNext k) :=
  ⟨rigidSphereRotation rigidGenerator x.val, by
    rw [← rigidSector_rotation]; exact mem_image_of_mem _ x.property⟩

theorem equivariantRigidSectorDisk_rotation (k : Fin 19) (x : rigidSector k) :
