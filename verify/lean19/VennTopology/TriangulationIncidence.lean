...
theorem diagram_encoded_link_preconnected (v : Fin (524288 + suppliedModel.oriented.size)) :
    IsPreconnected (Coordinates.link (encodedCells suppliedModel.oriented 524288) v.val) := by
  ...
  have he : Set.range (fun z : Circle => (e z).val) =
      dartLink suppliedModel.oriented 524288 v.val := by
    ext x
    constructor
    · rintro ⟨z, rfl⟩
      exact (e z).property
...
theorem diagram_finite_vertex_stars_connected :
  ...
  apply strongVertexStarConnected_of_links
    (fun t => cell (524288 + suppliedModel.oriented.size)
      (encodedCells suppliedModel.oriented 524288 t))
  · intro t
...
theorem diagram_finite_surface_incidence : diagramFiniteTriangulation.SurfaceIncidence := by
  ...
  · intro e he
    exact finite_encoded_edge_valence_le_two suppliedModel.oriented 524288
      (524288 + suppliedModel.oriented.size) suppliedLinkStars
...
theorem diagram_finite_no_boundary_edges (e : diagramFiniteTriangulation.Edge) :
  ...
  have hex : ∃ t, a.val ∈ encodedCells suppliedModel.oriented 524288 t ∧
      b.val ∈ encodedCells suppliedModel.oriented 524288 t := by
