...
theorem sector_corner_support (f : Fin suppliedModel.oriented.size) (i : Fin 4) :
    ...
    v < 524288 ∧ (suppliedSectorMasks.getD v 0).testBit (sectorColor f).val = true := by
...
def sectorAllowed (k : Fin 19) (a : Nat) : Prop :=
  (a < 524288 ∧ (suppliedSectorMasks.getD a 0).testBit k.val = true) ∨
  ∃ f : Fin suppliedModel.oriented.size, a = 524288 + f.val ∧ sectorColor f = k

theorem encodedSector_positive_allowed (k : Fin 19) {x : Point Nat}
    (hx : x ∈ encodedSector k) {a : Nat} (ha : 0 < x a) : sectorAllowed k a := by
  obtain ⟨t, ht, hx⟩ := mem_iUnion₂.mp hx
  have hm : a ∈ encodedCells suppliedModel.oriented 524288 t := by
    by_contra hn
...
theorem sector_overlap_edge (t : Fin (4*suppliedModel.oriented.size)) (l : Fin 19)
    ...
    ∃ j : Fin 38,
      ...
      ((suppliedSectorMasks.getD (LinkProof.triangleVertex suppliedModel.oriented 524288 t.val 1) 0).testBit l.val = true →
        LinkProof.triangleVertex suppliedModel.oriented 524288 t.val 1 = a ∨
        LinkProof.triangleVertex suppliedModel.oriented 524288 t.val 1 = b) ∧
      ((suppliedSectorMasks.getD (LinkProof.triangleVertex suppliedModel.oriented 524288 t.val 2) 0).testBit l.val = true →
        LinkProof.triangleVertex suppliedModel.oriented 524288 t.val 2 = a ∨
        LinkProof.triangleVertex suppliedModel.oriented 524288 t.val 2 = b) := by
  ...
  have hj' : j < 38 := by simpa using hj
  refine ⟨⟨j, hj'⟩, ?_⟩
  have hrow (i : Fin 38) :
  ...
  simp only [Array.getElem_range,
    show suppliedSectorColors.getD (t.val/4) 19 = (sectorColor (triangleFace _ t)).val from rfl,
    hrow ⟨j, hj'⟩, hrow ⟨(j+1)%38, Nat.mod_lt _ (by decide)⟩, Bool.and_eq_true] at he
  constructor
  · intro hm
...
theorem sector_inter_subset_boundary (k l : Fin 19) (hkl : k ≠ l) :
  ...
  have hz (a : Nat) (ha : a ∉ ({meridianBoundaryLabel k (cyclicNext k) j,
    ...
    · have hu := (sector_corner_support (triangleFace _ t) (triangleCorner _ t)).1
      have hb : LinkProof.triangleVertex suppliedModel.oriented 524288 t.val 1 < 524288 := by
    ...
    · have hu := (sector_corner_support (triangleFace _ t) (Quad.next (triangleCorner _ t))).1
      have hb : LinkProof.triangleVertex suppliedModel.oriented 524288 t.val 2 < 524288 := by
