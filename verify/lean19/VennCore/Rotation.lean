...
def regionStep (v : Nat) : Nat := if v < 2^19 then rotate v else v

def regionRotationCheck : Bool :=
  (Array.range (2^19)).all fun v => rotate v < 2^19 && steps regionStep 19 v == v
...
def faceRotationCheck (fs : Array Face) : Bool :=
  ...
  (Array.range fs.size).all fun f =>
    ...
    g < fs.size && s < 4 && steps (fun f => table.getD f 0) 19 f == f &&
      (Array.range 4).all fun k =>
        regionStep ((fs.getD f #[]).getD k 0) == (fs.getD g #[]).getD ((k+s)%4) 0

def trianglePole (u v : Nat) : Nat := if u == 0 || v == 0 then 0 else 524287
